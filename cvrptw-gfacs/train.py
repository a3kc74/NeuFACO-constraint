...
def calculate_log_pb_uniform(paths: torch.Tensor):
    ...
    log_b_p = - scipy.special.gammaln(n_routes + 1)
        ...
        wandb.log(
            {
                ...
                "train_mean_cost_ls": _train_mean_cost_ls / count,
                "train_min_cost_ls": _train_min_cost_ls / count,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output", type=str, default="../pretrained/cvrptw",
    ...
    if args.cost_w_min is None:
        args.cost_w_min = 0.5 if args.pretrained is None else 0.9
    ...
    run_name += f"{mode_name}{args.nodes}_sd{args.seed}"
    pretrained_name = (
    ...
    if USE_WANDB:
        wandb.init(project=f"gfacs-cvrptw", name=run_name)
        wandb.config.update(args)
        wandb.config.update({"T": T, "model": "GFACS", "tam": TAM, "vrptw": args.vrptw})
    ##################################################
