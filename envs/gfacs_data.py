...
def gen_instance(n, device, service_time=0.0, max_loc=150, max_time=480, tam=False, scale=True, vrptw=False):
    """
    ...
    demands_normalized = torch.zeros_like(demands) if vrptw else demands / get_capacity(n, tam)
...
def load_val_dataset(n_node, k_sparse, device, tam=False, vrptw=False):
    filename = f"../data/cvrptw/valDataset-{dataset_mode_prefix(tam, vrptw)}{n_node}.pt"
    if not os.path.isfile(filename):
        dataset = []
        for i in range(10):
            demand, dist, position, window = gen_instance(n_node, device, tam=tam, vrptw=vrptw)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--vrptw", action="store_true", help="Generate VRPTW data by setting all customer demands to zero")
    args = parser.parse_args()

    pathlib.Path('../data/cvrptw').mkdir(parents=True, exist_ok=True)
    ...
    for n in [100, 400, 1000]:  # problem scale
        ...
        inst_list = []
        for _ in range(100):
            demand, dist, position, window = gen_instance(n, 'cpu', tam=True, vrptw=args.vrptw)
            instance = torch.vstack([demand, position.T, dist, window.T])
    ...
    # for n in [50]:
        ...
        for _ in range(100):
            demand, dist, position, window = gen_instance(n, 'cpu', vrptw=args.vrptw)
            inst_list.append(torch.vstack([demand, position.T, dist, window.T]))
        test_dataset = torch.stack(inst_list)
        torch.save(test_dataset, f"../data/cvrptw/testDataset-{dataset_mode_prefix(vrptw=args.vrptw)}{n}.pt")
