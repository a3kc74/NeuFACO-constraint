...
from envs.gfacs_data import load_val_dataset, load_test_dataset
...
if __name__ == "__main__":
    ...
    if opt.type == "val":
        dataset = load_val_dataset(opt.nodes, opt.nodes // 5, "cpu", tam=opt.tam, vrptw=opt.vrptw)
    elif opt.type == "test":
        dataset = load_test_dataset(opt.nodes, opt.nodes // 5, "cpu", tam=opt.tam, vrptw=opt.vrptw)
    else:
    ...
    mode_suffix = f"{'-tam' if opt.tam else ''}{'-vrptw' if opt.vrptw else ''}"
    target_dir = os.path.join(opt.result_dir, f"{opt.type}Dataset{mode_suffix}-{opt.nodes}-{size}")
    if not os.path.isdir(target_dir):
        os.makedirs(target_dir)
    sol_file = os.path.join(target_dir, f"{opt.type}Dataset{mode_suffix}-{opt.nodes}-pyvrp_maxiter{opt.maxiter}.pkl")
    out_file = os.path.join(target_dir, f"{opt.type}Dataset{mode_suffix}-{opt.nodes}-pyvrp_maxiter{opt.maxiter}.txt")
